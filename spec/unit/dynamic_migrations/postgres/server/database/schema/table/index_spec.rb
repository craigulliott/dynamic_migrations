...
RSpec.describe DynamicMigrations::Postgres::Server::Database::Schema::Table::Index do
  ...
  describe :initialize do
    ...
    it "raises an error if providing something other than a symbol for the index name" do
      ...
      }.to raise_error DynamicMigrations::Postgres::Server::Database::Schema::Table::Index::InvalidNameError
    end

    it "raises an error if providing an index name which is too long" do
      expect {
        DynamicMigrations::Postgres::Server::Database::Schema::Table::Index.new :configuration, table, [column], :this_name_is_too_long_because_it_must_be_under_sixty_four_characters
      }.to raise_error DynamicMigrations::Postgres::Server::Database::Schema::Table::Index::InvalidNameError
    end
  end
