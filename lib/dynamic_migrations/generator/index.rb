  ...
  class Generator
    module Index
      def add_index index
        ...
        migration_syntax = where_sql + <<~RUBY
          add_index :#{index.table.name}, #{column_names}, #{options_syntax}
        RUBY

        add_migration index.table.schema.name, index.table.name, "add_index_#{index.name}_on_#{index.table.name}", migration_syntax
      end

      def remove_index index
        add_migration index.table.schema.name, index.table.name, "remove_#{index.name}_from_#{index.table.name}", <<~RUBY
